...
- **13**, `conflicts` in `mod.toml` (engine, 2026-09-23): a mod that replaces
  ...
  `provides` aliases too. The mod's manifest wants `conflicts = ["core_ui"]`;
  until then a local game with no `enabled_mods` still loads both.
