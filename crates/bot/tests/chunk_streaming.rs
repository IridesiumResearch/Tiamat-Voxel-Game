...
#[test]
fn a_mods_fog_reaches_the_client_by_both_paths() {
    // `game.register_chunk_fog` through a real server, the way the tint is
    // held above — and by BOTH ways a chunk is served, because they ask for it
    // in different places. Alice's chunks are generated by the workers, which
    // compute the fog beside the terrain; Bob joins after, onto chunks already
    // resident, which the tick serves by asking `Generator::fog` — the seam
    ...
    block_on(async {
        let mut alice = join(&server, "Alice").await;
        collect_until_quiet(&mut alice).await;
        check("Alice", alice.chunk_fogs_received());

        let mut bob = join(&server, "Bob").await;
        collect_until_quiet(&mut bob).await;
        check("Bob", bob.chunk_fogs_received());
